// ============================================================================
// 老友记（wybcs.work）每日签到脚本，用于 Quantumult X 定时任务
// ----------------------------------------------------------------------------
// 功能：
//   1. 调用登录接口 https://emcs.605305.xyz/api/requests/auth
//      从响应 Set-Cookie 中读取 session_id 并缓存
//   2. 调用签到接口 https://emcs.605305.xyz/api/user/points/checkin
//      使用 session_id 作为 Cookie 授权
//   3. 签到成功后自动调用积分兑换接口 https://emcs.605305.xyz/api/user/points/redeem
//   4. 执行结果通过 Quantumult X 通知和 $done 弹窗输出
//
// 使用说明：
//   1. 修改下面的 DEFAULT_USERNAME / DEFAULT_PASSWORD
//   2. 在 Quantumult X [task_local] 配置中添加定时任务，例如：
//      45 8 * * * https://raw.githubusercontent.com/tool-config/cron_script/wybcs.js, tag=老友记签到, img-url=https://emcs.605305.xyz/static/img/logo-app-2.png
//
// 脚本图标：
//   https://emcs.605305.xyz/static/img/logo-app-2.png
// ============================================================================

// 这里必须填写 wybcs.work 的账号密码。
// 也可以改用 QX 的 $prefs 值：taotu_username / taotu_password。
const DEFAULT_USERNAME = 'guosir';
const DEFAULT_PASSWORD = 'FVzpxKTQpSU';

const BASE_URL = 'https://emcs.605305.xyz';
